...
class Language {
	...
	public function get($clef, $lang){
		...
		$lang = $this->manager->getFromLang(new \Library\Entities\Language(array("clef" => $clef, "lang" => $lang)));
		
		if ($lang != null) {
			return $lang->valeur();
		} else {
			$lang = $this->manager->getFromLang(new \Library\Entities\Language(array("clef" => $clef, "lang" => \Utils::defaultLanguage())));
			if ($lang != null)
				return $lang->valeur();
			else
				return null;
