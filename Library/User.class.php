...
class User extends ApplicationComponent{
	...
	public function isAdmin(){
		return (($lvl = \Library\Application::appConfig()->getConst("ADMIN_LVL")) !== NULL && $lvl <= $this->getAdminLvl());
	}
	...
	public function getFlash(){
		...
		if(defined($flash)){
			...
			if(($autoFlash = $this->app()->language()->get($flash, $this->app()->httpRequest()->languageGet())) != null) {
				return $autoFlash;
			} else {
				return $flash;
			}
	...
	public function connect(\Library\Entities\User $pUser, $attribute) {
		...
		$_SESSION["connected_attr"] = array_keys($attributes);
